...
export const TEST_RECIPES = {
  ...
  PROTEIN_SHAKE: {
    ...
    steps: ["Wszystkie składniki zmiksuj w blenderze", "Podawaj od razu po przygotowaniu"],
    nutrition: {
      kcal: 380,
      protein_g: 15,
      fat_g: 18,
  ...
  async createRecipe(command: RecipeCreateCommand): Promise<RecipeDTO> {
    ...
    const { data: recipe, error: recipeError } = await this.supabase
      ...
      .insert({
        ...
        ingredients:
          validatedData.ingredients as unknown as Database["public"]["Tables"]["recipes"]["Insert"]["ingredients"],
        steps:
          validatedData.steps as unknown as Database["public"]["Tables"]["recipes"]["Insert"]["steps"],
        kcal: 0,
        protein_g: 0,
        fat_g: 0,
        carbs_g: 0,
    ...
    try {
      const nutritionValues = await nutritionService.calculateNutrition(
